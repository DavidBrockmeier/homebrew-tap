...
CI builds and tests the same-named formula on an ARM64 macOS 26 runner, with and
without the private-API option. The formula test checks its installed ICD path,
creates a Vulkan instance, enumerates a GPU, and destroys the instance. It is an
initialization test, not an interpolation benchmark or a complete Vulkan CTS run.
